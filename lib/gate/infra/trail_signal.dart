...
/// Assert-wrapped tracer — the closure AND its string literals are stripped
/// from release builds so no `[HD.*]` tag ever ships.
void gateTrace(String Function() message) {
  assert(() {
    debugPrint(message());
    return true;
  }());
...
/// Owns AppsFlyer: ATT prompt, conversion/reopen/deep-link capture, the Organic
/// false-positive recheck via GCD, and building the flat config-request body.
class TrailSignal {
  TrailSignal(this._agent);
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final sdk = AppsflyerSdk(
        AppsFlyerOptions(
          afDevKey: DashGateConfig.appsFlyerKey,
          appId: DashGateConfig.iosStoreId,
      ...
      gateTrace(() => '[HD.TRAIL] init failed: $error');
      _completeEmpty();
    }
  }
  ...
  Future<void> _acceptInstall(dynamic raw) async {
    try {
      ...
      gateTrace(
        () => '[HD.TRAIL] conversion status=$status '
      ...
      if (failed) {
        ...
        await Future<void>.delayed(
          const Duration(seconds: DashGateConfig.organicRecheckSeconds),
        );
      ...
      gateTrace(() => '[HD.TRAIL] conversion parse error: $error');
      _install = <String, dynamic>{};
    } finally {
  ...
  Future<Map<String, dynamic>?> _fetchGcd() async {
    ...
    try {
      ...
      final base = DashGateConfig.gcdBase;
      final sep = base.contains('?') ? '&' : '?';
      final uri = Uri.parse(
        '$base${sep}app_id=${DashGateConfig.iosStoreId}&device_id=$uid',
      );
      final response = await _agent
          .get(
            uri,
            headers: <String, String>{
              'Authorization': 'Bearer ${DashGateConfig.appsFlyerKey}',
            },
          )
  ...
  Future<Map<String, dynamic>> compose({
    ...
    body['bundle_id'] = DashGateConfig.bundleId;
    body['os'] = 'iOS';
    body['store_id'] = DashGateConfig.storeToken;
    body['locale'] = locale;
    if (pushToken != null &&
        pushToken.isNotEmpty &&
        DashGateConfig.firebaseProjectNumber.isNotEmpty) {
      body['push_token'] = pushToken;
      body['firebase_project_id'] = DashGateConfig.firebaseProjectNumber;
    }
    ...
    gateTrace(() => '[HD.TRAIL] payload ${jsonEncode(body)}');
    return body;
  }
